class Scene4 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("scene4tiles", "images/New Piskel (5).png");
        this.load.tilemapCSV("scene4", "scene-4.csv");
    }

    create() {
        this.baseTilemap = this.make.tilemap(
            {key: "scene4", tileWidth: 32, tileHeight: 32});
        var world1tiles = this.baseTilemap.addTilesetImage("scene4tileset",
            "scene4tiles");
    ...
    update() {
        ...
        if (Math.abs(710 - this.player.sprite.x) < 50 && Math.abs(225 - this.player.sprite.y) < 50) {
            this.scene.stop("scene-4");
            this.scene.start("scene-5");
